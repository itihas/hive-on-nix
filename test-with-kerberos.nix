...
let
  ...
  extraConfDirs = pkgs.lib.mapAttrsToList (n: v: siteXml "ssl-${n}.xml" v) {
      server = {
...
makeTest
{
	...
	nodes = {
		...
		nn1 = { pkgs, ... }: {
			...
			services.hadoop = {
				inherit package coreSite hdfsSite extraConfDirs;
				kerberos = {
					enable = true;
					realm = "TEST.REALM";
				};
		...
		jn1 = { ... }: {
			...
			networking.firewall.allowedTCPPorts = [ 8481 ];
		...
		dn1 = { pkgs, ... }: {
			...
			networking.firewall.allowedTCPPorts = [ 9866 9865 9864 1004 1006 ];
      systemd.services.hdfs-datanode = {
		...
		rm1 = { config, options, ... }: {
			...
			services.hadoop = {
				inherit package coreSite hdfsSite yarnSite extraConfDirs;
				kerberos = {
					enable = true;
					realm = "TEST.REALM";
				};
